...
<table width='100%' align='center' style='font-size: 13px;'
	...
	<tr>
		<td
			...
			<c:choose>
				<c:when test="${pageModel.pageNum==1 && pageModel.pages<=1}">
				...
				<c:when test="${pageModel.pageNum==pageModel.pages}">
					<span class='current'><a href='#'
						onclick="toPage(${pageModel.pageNum-1})">上一页</a></span>
					<c:forEach begin="1" end="${pageModel.pages}" var="i">
						<c:choose>
							<c:when test="${pageModel.pageNum==i}">
								<span class='current'>${i}</span>
				...
				<c:otherwise>
					<span class='current'><a href='#' onclick="toPage(${pageModel.pageNum-1})" >上一页</a></span>
					<c:forEach begin="1" end="${pageModel.pages}" var="i">
						<c:choose>
							<c:when test="${pageModel.pageNum==i}">
								<span class='current'>${i}</span>
			...
			type='text' size='2' id='pager_jump_page_size' value="${pageModel.pageNum }" /> &nbsp;<input
			type='button'
